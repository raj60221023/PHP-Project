$(document).ready(function () {
	...
	*/
	...
	$("#login").on("submit", function (event) {
		...
		$.ajax({
			...
			dataType: "json", // Now expecting JSON response
			success: function (data) {
				$(".overlay").hide();
				if (data.status == "success") {
					window.location.href = "index.php";
				} else if (data.status == "cart_login") {
					window.location.href = "cart.php";
				} else {
					// Use SweetAlert if available, otherwise standard error message
					if (typeof swal !== 'undefined') {
						swal("Login Failed", data.message, "error");
					} else {
						$("#e_msg").html(data.message).css("color", "red");
					}
				}
			},
			error: function (xhr, status, error) {
				$(".overlay").hide();
				$("#e_msg").html("Server Error. Please try again later.").css("color", "red");
	...
	$("body").delegate(".add-to-cart-btn", "click", function (event) {
		...
		var qty = 1;
		if ($(this).closest('.product-details').length > 0 && $("#qty_num").length > 0) {
			qty = $("#qty_num").val();
		}
	...
	*/
	...
	// Compare Logic
	$("body").delegate(".add-to-compare", "click", function (event) {
		var pid = $(this).attr("pid");
		event.preventDefault();
		$.ajax({
			url: "action.php",
			method: "POST",
			data: { addToCompare: 1, proId: pid },
			success: function (data) {
				swal("Compare", data, "success");
			}
		})
	})
})