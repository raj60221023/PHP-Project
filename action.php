...
if(isset($_POST["category"])){
    ...
    if(count($rows) > 0){
        ...
        foreach($rows as $row){
            ...
            $countStmt = $con->prepare("SELECT COUNT(*) AS count_items FROM products WHERE product_cat=?");
            $countStmt->execute([$i]);
            $cRow = $countStmt->fetch();
            $count = $cRow["count_items"];
            $i++;
            echo "
...
if(isset($_POST["brand"])){
    ...
    if(count($rows) > 0){
        ...
        foreach($rows as $row){
            ...
            $countStmt = $con->prepare("SELECT COUNT(*) AS count_items FROM products WHERE product_brand=?");
            $countStmt->execute([$i]);
            $cRow = $countStmt->fetch();
            $count=$cRow["count_items"];
            $i++;
...
if(isset($_POST["page"])){
    $stmt = $con->query("SELECT COUNT(*) AS c FROM products");
    $countRow = $stmt->fetch();
    $count = $countRow ? (int)$countRow['c'] : 0;
    $pageno = $count > 0 ? ceil($count/9) : 0;
    for($i=1;$i<=$pageno;$i++){
        echo "
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
    if(isset($_POST["get_seleted_Category"])){
        $id = $_POST["cat_id"];
        $stmt = $con->prepare("SELECT * FROM products,categories WHERE product_cat = ? AND product_cat=cat_id");
        $stmt->execute([$id]);
        $rows = $stmt->fetchAll();
    }else if(isset($_POST["selectBrand"])){
        $id = $_POST["brand_id"];
        $stmt = $con->prepare("SELECT * FROM products,categories WHERE product_brand = ? AND product_cat=cat_id");
        $stmt->execute([$id]);
        $rows = $stmt->fetchAll();
    }else {
        $keyword = $_POST["keyword"];
        $search_term = "%" . $keyword . "%";
        $stmt = $con->prepare("SELECT * FROM products,categories WHERE product_cat=cat_id AND product_keywords LIKE ?");
	...
	if(isset($_SESSION["uid"])){
		...
		$check = $con->prepare("SELECT id, qty FROM cart WHERE p_id = ? AND user_id = ?");
		...
		if($check->rowCount() > 0){
			...
			$ins = $con->prepare("INSERT INTO cart (p_id, ip_add, user_id, qty) VALUES (?, ?, ?, ?)");
			if($ins->execute([$p_id, $ip_add, $user_id, $qty])){
				echo "Product is Added..!";
			}
		}
		}else{
			$check = $con->prepare("SELECT id, qty FROM cart WHERE ip_add = ? AND p_id = ? AND user_id = -1");
			$check->execute([$ip_add, $p_id]);
            $row = $check->fetch();
			if ($check->rowCount() > 0) {
                $new_qty = $row['qty'] + $qty;
                $update = $con->prepare("UPDATE cart SET qty = ? WHERE id = ?");
			...
			$ins = $con->prepare("INSERT INTO cart (p_id, ip_add, user_id, qty) VALUES (?, ?, -1, ?)");
			if ($ins->execute([$p_id, $ip_add, $qty])) {
				echo "Product is Added..!";
				exit();
			}
...
if (isset($_POST["count_item"])) {
    if (isset($_SESSION["uid"])) {
        ...
        $stmt = $con->prepare("SELECT COUNT(*) AS count_item FROM cart WHERE ip_add = ? AND user_id < 0");
        $stmt->execute([$ip_add]);
    }
    $row = $stmt->fetch();
    echo $row ? $row["count_item"] : 0;
    exit();
}
//Count User cart item
...
if (isset($_POST["Common"])) {
    ...
    if (isset($_SESSION["uid"])) {
        $stmt = $con->prepare("SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id=?");
        $stmt->execute([$_SESSION["uid"]]);
    }else{
        $stmt = $con->prepare("SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add=? AND b.user_id < 0");
        $stmt->execute([$ip_add]);
    }
    $rows = $stmt->fetchAll();
                ...
                if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="checkout.php" method="post">
							...
							$x=0;
                            $stmt2 = $con->prepare("SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id=?");
...
if (isset($_POST["removeItemFromCart"])) {
    $remove_id = $_POST["rid"];
    if (isset($_SESSION["uid"])) {
        $stmt = $con->prepare("DELETE FROM cart WHERE p_id = ? AND user_id = ?");
        $ok = $stmt->execute([$remove_id, $_SESSION["uid"]]);
    }else{
        $stmt = $con->prepare("DELETE FROM cart WHERE p_id = ? AND ip_add = ?");
        $ok = $stmt->execute([$remove_id, $ip_add]);
    }
    if($ok){
...
if (isset($_POST["updateCartItem"])) {
    ...
    $qty = $_POST["qty"];
    if (isset($_SESSION["uid"])) {
        $stmt = $con->prepare("UPDATE cart SET qty=? WHERE p_id = ? AND user_id = ?");
        $ok = $stmt->execute([$qty, $update_id, $_SESSION["uid"]]);
    }else{
        $stmt = $con->prepare("UPDATE cart SET qty=? WHERE p_id = ? AND ip_add = ?");
        $ok = $stmt->execute([$qty, $update_id, $ip_add]);
    }
    if($ok){
...
if(isset($_POST["addToCompare"])){
    $pid = $_POST["proId"];
    if(!isset($_SESSION["compare_ids"])){
        $_SESSION["compare_ids"] = array();
    }
    if(in_array($pid, $_SESSION["compare_ids"])){
        echo "Product is already in compare list!";
    } else {
        if(count($_SESSION["compare_ids"]) >= 3){
             array_shift($_SESSION["compare_ids"]);
        }
        $_SESSION["compare_ids"][] = $pid;
